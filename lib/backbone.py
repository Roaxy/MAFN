...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
        ...
        self.relation_judger = nn.Sequential(
            Mlp_2(dim, dim, dim), torch.nn.Softmax(dim=-1)
        )
    ...
    def forward(self, x, mask_matrix):
        ...
        x = x.view(B, H * W, C)
        # adpative noise in cross-atten
        new_x=[]
        for bs in range(B):
            q = x[bs].unsqueeze(0)
            judger_input=x[bs].unsqueeze(0)
            ...
            v = torch.cat([noise_v, x[bs].unsqueeze(0)], dim=0)

            new_x.append(x[bs] + self.cross_attn_0_to_1(q, k, v)[0].squeeze(0))
        x = torch.stack(new_x)
...
class MultiModalSwinTransformer(nn.Module):
    ...
    def forward(self, x, l, l_mask):
        """Forward function."""
        x = self.patch_embed(x) ##(2,3,480,480)->(2,128,120,120)
        ...
        if self.ape:
            ...
            ##enter this
            x = x.flatten(2).transpose(1, 2)#(2,128,120,120)->(2,14400,128)
        x = self.pos_drop(x)
...
class MMBasicLayer(nn.Module):
    def __init__(self,
        ...
        self.corr=CorrelationAware(dim, dim, use_corr=True, use_checkpoint=use_checkpoint)
    ...
    def forward(self, x, H, W, l, l_mask):
        ...
        a=self.visual_gate(v_residual)
        b=self.visual_gate(v_residual) * v_residual
        # apply a gate on the residual
        x_out=self.corr(self.visual_gate(v_residual) * v_residual ,self.res_gate(x_residual) * x_residual)
        x = x + x_out 
...
class CorrelationAware(nn.Module):  #input shape: n, c, h, w， d
    """Correlation-aware Fusion block."""
    ...
    def __init__(self, in_channels, num_channels, use_corr=True, use_checkpoint=False):
        ...
        self.channel_intensification_block = RCAB(num_channels, use_checkpoint=use_checkpoint)
    
    def forward(self, x_1, x_2):
        bs, n, dim = x_1.shape
        h, w = int(np.sqrt(n)), int(np.sqrt(n))
        x_1 = x_1.view(bs, h, w, dim).permute(0, 3, 1, 2)
        x_2 = x_2.view(bs, h, w, dim).permute(0, 3, 1, 2)
        if self.use_corr:
        ...
        shortcut = x
        x = x.permute(0,2,3,1)  #n,h,w,d,c
        x = self.mlpLayer(x)
        x = self.channel_intensification_block(x)
        x = x.permute(0,3,1,2)  #n,c,h,w,d
        
        x_out = x + shortcut
        
        return x_out.view(bs, dim, -1).permute(0, 2, 1)
...
class CALayer(nn.Module):  #input shape: n, h, w, c
    ...
    def __init__(self, num_channels, reduction=2, use_bias=True):
        super().__init__()
    ...
    def forward(self, x_in):
        ...
        x = x_in.permute(0,3,1,2)  #n,c,h,w,d
...
class WinGmlpLayer(nn.Module):  #input shape: n, h, w, c
    ...
    def forward(self, x):
        ...
        x = F.pad(x, (0, 0,pad_t, pad_b, pad_d0, pad_d1))
        
        gh, gw = x.shape[1] // self.fh, x.shape[2] // self.fw
        x = split_images(x, patch_size=(self.fh, self.fw))  #n (gh gw gd) (fh fw fd) c
