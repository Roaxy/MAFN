...
def evaluate(model, data_loader, bert_model, epoch):
    ...
    if args.local_rank == 0 and not args.debug:
        wandb.log({
            ...
            "precision@0.5":seg_correct[0] * 100. / seg_total,
            "precision@0.6":seg_correct[1] * 100. / seg_total,
            "precision@0.7":seg_correct[2] * 100. / seg_total,
            "precision@0.8":seg_correct[3] * 100. / seg_total,
            "precision@0.9":seg_correct[4] * 100. / seg_total,})
...
def main(args):
    ...
    if not args.debug:
        if utils.is_main_process():
            wandb.init(project='WQN',name=args.output_dir)
    else:
    ...
    for epoch in range(max(0, resume_epoch+1), args.epochs):
        ...
        if epoch == args.epochs-1 :
            print("Best results:")
            checkpoint = torch.load(os.path.join(args.output_dir, 'model_best_RMSIN.pth'), map_location='cpu')
            single_model.load_state_dict(checkpoint['model'], strict=False)
            if args.model != 'lavt_one':
                single_bert_model.load_state_dict(checkpoint['bert_model'])
            iou, overallIoU = evaluate(model, data_loader_test, bert_model, epoch)
